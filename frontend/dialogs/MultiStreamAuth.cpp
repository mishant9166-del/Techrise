	...
	connect(reply, &QNetworkReply::finished, this, [this, reply]() {
		if (reply->error() == QNetworkReply::NoError) {
			...
			if (obj.contains("refresh_token"))
				session.refreshToken = obj["refresh_token"].toString();
			if (obj.contains("expires_in"))
				session.expiryTime = QDateTime::currentSecsSinceEpoch() + obj["expires_in"].toInt();
			...
			QMessageBox::critical(nullptr, "Token Exchange Error",
...
static void on_custom_output_starting(void *data, calldata_t *cd)
{
	...
	PlatformSessionManager::Get()->platformStates[client->getPlatform()] =
		PlatformSessionManager::OutputState::Starting;
}
...
static void on_custom_output_start(void *data, calldata_t *cd)
{
	...
	OAuthClient *client = static_cast<OAuthClient *>(data);
	PlatformSessionManager::Get()->platformStates[client->getPlatform()] =
		PlatformSessionManager::OutputState::Active;
}
...
static void on_custom_output_stop(void *data, calldata_t *cd)
{
	...
	OAuthClient *client = static_cast<OAuthClient *>(data);
	PlatformSessionManager::Get()->platformStates[client->getPlatform()] =
		PlatformSessionManager::OutputState::Stopped;
}
...
static void on_custom_output_reconnect(void *data, calldata_t *cd)
{
	...
	OAuthClient *client = static_cast<OAuthClient *>(data);
	PlatformSessionManager::Get()->platformStates[client->getPlatform()] =
		PlatformSessionManager::OutputState::Reconnecting;
}
...
void PlatformSessionManager::OBSFrontendEvent(enum obs_frontend_event event, void *private_data)
{
	...
	blog(LOG_INFO, "[MultiStream] OBSFrontendEvent triggered: %d, mainStreamPlatform before: %s", (int)event,
	     manager->mainStreamPlatform.toUtf8().constData());
	if (event == OBS_FRONTEND_EVENT_STREAMING_STARTING) {
		if (manager->mainStreamPlatform.isEmpty()) {
			...
			if (service) {
				...
				if (url.contains("twitch", Qt::CaseInsensitive))
					manager->mainStreamPlatform = "Twitch";
				else if (url.contains("youtube", Qt::CaseInsensitive))
					manager->mainStreamPlatform = "YouTube";
				else if (url.contains("facebook", Qt::CaseInsensitive))
					manager->mainStreamPlatform = "Facebook";
				obs_data_release(settings);
				obs_service_release(service);
			}
		}
		blog(LOG_INFO, "[MultiStream] STREAMING_STARTING, mainStreamPlatform after deduction: %s",
		     manager->mainStreamPlatform.toUtf8().constData());
		if (!manager->mainStreamPlatform.isEmpty()) {
			manager->platformStates[manager->mainStreamPlatform] = OutputState::Starting;
			OAuthClient *mainClient = nullptr;
			if (manager->mainStreamPlatform == "Twitch")
				mainClient = manager->twitchClient;
			else if (manager->mainStreamPlatform == "YouTube")
				mainClient = manager->youtubeClient;
			else if (manager->mainStreamPlatform == "Facebook")
				mainClient = manager->facebookClient;
		...
		blog(LOG_INFO, "[MultiStream] STREAMING_STARTED, mainStreamPlatform: %s",
		     manager->mainStreamPlatform.toUtf8().constData());
		if (!manager->mainStreamPlatform.isEmpty()) {
		...
		blog(LOG_INFO, "[MultiStream] STREAMING_STOPPING, mainStreamPlatform: %s",
		     manager->mainStreamPlatform.toUtf8().constData());
		if (!manager->mainStreamPlatform.isEmpty()) {
			manager->platformStates[manager->mainStreamPlatform] = OutputState::Stopped;
		}
	} else if (event == OBS_FRONTEND_EVENT_STREAMING_STOPPED) {
		blog(LOG_INFO, "[MultiStream] STREAMING_STOPPED, mainStreamPlatform: %s",
		     manager->mainStreamPlatform.toUtf8().constData());
		if (!manager->mainStreamPlatform.isEmpty()) {
			manager->platformStates[manager->mainStreamPlatform] = OutputState::Stopped;
		}
...
void PlatformSessionManager::startPlatformOutput(OAuthClient *client, const QString &rtmpUrl, const QString &streamKey)
{
	...
	if (platName == mainStreamPlatform) {
		blog(LOG_INFO, "[MultiStream] startPlatformOutput: Skipped %s because it is the main stream",
	...
	obs_output_t *out =
		obs_output_create("rtmp_output", (platName + "_output").toUtf8().constData(), nullptr, nullptr);
	if (!out) {
		blog(LOG_ERROR, "[MultiStream] startPlatformOutput: FAILED to create rtmp_output for %s!",
		     platName.toUtf8().constData());
		return;
	}
	...
	obs_service_t *service =
		obs_service_create("rtmp_custom", (platName + "_service").toUtf8().constData(), nullptr, nullptr);
	if (service) {
		blog(LOG_INFO, "[MultiStream] startPlatformOutput: successfully created rtmp_custom service for %s",
		     platName.toUtf8().constData());
		...
		blog(LOG_ERROR, "[MultiStream] startPlatformOutput: FAILED to create rtmp_custom service for %s!",
		     platName.toUtf8().constData());
	}
	...
	if (mainOut) {
		...
		if (v_enc)
			obs_output_set_video_encoder(out, v_enc);
		if (a_enc)
			obs_output_set_audio_encoder(out, a_enc, 0);

		bool v_active = v_enc ? obs_encoder_active(v_enc) : false;
		bool a_active = a_enc ? obs_encoder_active(a_enc) : false;

		blog(LOG_INFO,
		     "[MultiStream] startPlatformOutput: setting encoders from mainOut (v_enc: %p [active:%s], a_enc: %p [active:%s]). URL: %s",
	...
	if (currentService) {
		...
		blog(LOG_INFO, "[MultiStream] startPlatformOutput: obs_service_can_try_to_connect returned %s",
	...
	blog(LOG_INFO, "[MultiStream] startPlatformOutput: obs_output_start for %s returned %s. Last error: %s",
	     platName.toUtf8().constData(), success ? "true" : "false",
	...
	if (client->getSession().streamEnabled && client->isLoggedIn()) {
		client->prepareStream([this, client](QString url, QString key) {
			if (!obs_frontend_streaming_active()) {
				...
				obs_service_t *service =
					obs_service_create("rtmp_custom", "main_custom_service", nullptr, nullptr);
				if (service) {
					obs_data_t *settings = obs_data_create();
...
void PlatformSessionManager::startCustomStreams()
{
	if (twitchClient->getSession().streamEnabled && twitchClient->isLoggedIn()) {
		twitchClient->prepareStream(
			[this](QString url, QString key) { startPlatformOutput(twitchClient, url, key); });
	}
	if (youtubeClient->getSession().streamEnabled && youtubeClient->isLoggedIn()) {
		youtubeClient->prepareStream(
			[this](QString url, QString key) { startPlatformOutput(youtubeClient, url, key); });
	}
	if (facebookClient->getSession().streamEnabled && facebookClient->isLoggedIn()) {
		facebookClient->prepareStream(
			[this](QString url, QString key) { startPlatformOutput(facebookClient, url, key); });
	}
}
...
QString TwitchOAuthClient::getAuthUrl() const
{
	return QString("https://id.twitch.tv/oauth2/authorize?client_id=%1&redirect_uri=http://localhost:8080/&response_type=code&scope=user:read:email+channel:read:stream_key+channel:manage:broadcast")
		.arg(clientId);
}
QString TwitchOAuthClient::getTokenUrl() const
{
...
void TwitchOAuthClient::prepareStream(std::function<void(QString, QString)> callback)
{
	...
	if (!session.streamCategory.isEmpty()) {
		QNetworkRequest searchReq(QUrl(QString("https://api.twitch.tv/helix/search/categories?query=%1")
						       .arg(QString(QUrl::toPercentEncoding(session.streamCategory)))));
		...
		connect(searchRep, &QNetworkReply::finished, this, [this, searchRep]() {
			...
			QNetworkRequest updateReq(QUrl(QString("https://api.twitch.tv/helix/channels?broadcaster_id=%1")
							       .arg(session.channelId)));
			...
			QNetworkReply *updateRep = networkManager->sendCustomRequest(
				updateReq, "PATCH", QJsonDocument(updateData).toJson());
			connect(updateRep, &QNetworkReply::finished, updateRep, &QObject::deleteLater);
		});
	} else {
		QNetworkRequest updateReq(
			QUrl(QString("https://api.twitch.tv/helix/channels?broadcaster_id=%1").arg(session.channelId)));
		updateReq.setRawHeader("Client-Id", clientId.toUtf8());
		updateReq.setRawHeader("Authorization", QString("Bearer %1").arg(session.accessToken).toUtf8());
		updateReq.setHeader(QNetworkRequest::ContentTypeHeader, "application/json");
		...
		QJsonObject updateData;
		updateData["title"] = session.streamTitle;
		QNetworkReply *updateRep =
			networkManager->sendCustomRequest(updateReq, "PATCH", QJsonDocument(updateData).toJson());
		connect(updateRep, &QNetworkReply::finished, updateRep, &QObject::deleteLater);
	}

	QNetworkRequest request(
		QUrl(QString("https://api.twitch.tv/helix/streams/key?broadcaster_id=%1").arg(session.channelId)));
	request.setRawHeader("Client-Id", clientId.toUtf8());
	request.setRawHeader("Authorization", QString("Bearer %1").arg(session.accessToken).toUtf8());
...
QString YouTubeOAuthClient::getAuthUrl() const
{
	return QString("https://accounts.google.com/o/oauth2/v2/auth?client_id=%1&redirect_uri=http://localhost:8080/&response_type=code&scope=https://www.googleapis.com/auth/youtube.readonly+https://www.googleapis.com/auth/youtube")
		.arg(clientId);
}
QString YouTubeOAuthClient::getTokenUrl() const
{
...
{
	QNetworkRequest request(
		QUrl("https://www.googleapis.com/youtube/v3/channels?part=snippet,statistics&mine=true"));
	request.setRawHeader("Authorization", QString("Bearer %1").arg(session.accessToken).toUtf8());
	QNetworkReply *reply = networkManager->get(request);
	connect(reply, &QNetworkReply::finished, this, [this, reply]() {
		if (reply->error() == QNetworkReply::NoError) {
			...
			if (obj.contains("items") && obj["items"].toArray().size() > 0) {
				...
				if (snippet.contains("thumbnails") &&
				    snippet["thumbnails"].toObject().contains("default")) {
					session.profilePicture =
						snippet["thumbnails"].toObject()["default"].toObject()["url"].toString();
				}
...
void YouTubeOAuthClient::prepareStream(std::function<void(QString, QString)> callback)
{
	...
	QNetworkRequest req(QUrl(
		"https://youtube.googleapis.com/youtube/v3/liveBroadcasts?part=snippet,status,contentDetails&broadcastType=all&mine=true"));
	...
	connect(rep, &QNetworkReply::finished, this, [this, rep, callback]() {
		...
		if (rep->error() == QNetworkReply::NoError) {
			...
			if (obj.contains("items") && obj["items"].toArray().size() > 0) {
				...
				if (broadcast.contains("contentDetails")) {
					boundStreamId =
						broadcast["contentDetails"].toObject()["boundStreamId"].toString();
				}
				...
				if (cat == "gaming")
					...
					catId = "23";
				else if (cat == "news & politics" || cat == "news")
					catId = "25";
				...
				QNetworkRequest updateReq(QUrl(
					"https://youtube.googleapis.com/youtube/v3/liveBroadcasts?part=snippet,status"));
				updateReq.setRawHeader("Authorization",
						       QString("Bearer %1").arg(session.accessToken).toUtf8());
				updateReq.setHeader(QNetworkRequest::ContentTypeHeader, "application/json");

				QNetworkReply *updateRep =
					networkManager->put(updateReq, QJsonDocument(updateData).toJson());
				...
				QNetworkRequest vidReq(
					QUrl("https://youtube.googleapis.com/youtube/v3/videos?part=snippet&id=" +
					     broadcastId));
				vidReq.setRawHeader("Authorization",
						    QString("Bearer %1").arg(session.accessToken).toUtf8());
				...
				connect(vidRep, &QNetworkReply::finished, vidRep, [this, vidRep, broadcastId, catId]() {
					if (vidRep->error() == QNetworkReply::NoError) {
						...
						if (vObj.contains("items") && vObj["items"].toArray().size() > 0) {
							...
							QNetworkRequest vUpReq(QUrl(
								"https://youtube.googleapis.com/youtube/v3/videos?part=snippet"));
							vUpReq.setRawHeader(
								"Authorization",
								QString("Bearer %1").arg(session.accessToken).toUtf8());
							vUpReq.setHeader(QNetworkRequest::ContentTypeHeader,
									 "application/json");

							QNetworkReply *vUpRep = networkManager->put(
								vUpReq, QJsonDocument(vUpdateData).toJson());
							connect(vUpRep, &QNetworkReply::finished, vUpRep,
								&QObject::deleteLater);
						}
					}
					vidRep->deleteLater();
		...
		if (!boundStreamId.isEmpty()) {
			streamReqUrl =
				"https://youtube.googleapis.com/youtube/v3/liveStreams?part=cdn&id=" + boundStreamId;
		}
...
QString FacebookOAuthClient::getAuthUrl() const
{
	return QString("https://www.facebook.com/v17.0/dialog/oauth?client_id=%1&redirect_uri=http://localhost:8080/&response_type=code&scope=email,pages_show_list,pages_read_engagement,pages_manage_posts,publish_video")
		.arg(clientId);
}
QString FacebookOAuthClient::getTokenUrl() const
{
...
void FacebookOAuthClient::fetchProfile()
{
	...
	QNetworkRequest request(
		QUrl(QString("https://graph.facebook.com/me?fields=id,name,email,picture&access_token=%1")
	...
	connect(reply, &QNetworkReply::finished, this, [this, reply]() {
		if (reply->error() == QNetworkReply::NoError) {
			...
			// 2. Fetch Pages
			QNetworkRequest pageReq(QUrl(QString("https://graph.facebook.com/me/accounts?access_token=%1")
							     .arg(session.accessToken)));
				...
				if (pageReply->error() == QNetworkReply::NoError) {
					...
					if (pObj.contains("data")) {
						...
						for (int i = 0; i < data.size(); i++) {
							...
							pagesList << page["name"].toString() + "|" +
									     page["id"].toString() + "|" +
									     page["access_token"].toString();
						}
...
void FacebookOAuthClient::prepareStream(std::function<void(QString, QString)> callback)
{
	...
	if (!session.streamTitle.isEmpty())
		params.addQueryItem("title", session.streamTitle);
	if (!session.streamDescription.isEmpty())
		params.addQueryItem("description", session.streamDescription);
